...
const Project = ({url, img, title}) => {
  return (
    <a href={url} target="_blank" className='w-full aspect-video overflow-hidden relative group transition-all rounded'>
        ...
        <div className='w-full h-full absolute bg-neutral-900/90 top-0 left-0 hidden group-hover:flex flex-col justify-center items-center Z-20 transition-all'>
            <h2 className='text-neutral-100 font-bold text-center'>{title}</h2>
        </div>
    </a>
  )
}
